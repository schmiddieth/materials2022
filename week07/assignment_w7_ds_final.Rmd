...
#restrict the sites to only overlapping peaks with GCR peak
...
moi <- runFimo(chr19, convert_motifs(motif_GCR), meme_path = "/mnt/IM/conda/bin/")
...
motifFootprint <- function(bamfile, motifs_GRC, motif_occurences, genome, around=100){
  seqlengths(motif_occurences) <- seqlengths(genome)[seqlevels(motif_occurences)]
  seqlevelsStyle(motif_occurences) <- "UCSC"
  ATACseqQC::factorFootprints(bamfile, pfm=motifs_GRC, bindingSites=motif_occurences, 
                              upstream=around, downstream=around,
                              seqlev=seqlevels(motif_occurences))
}
motifFootprint("atac.chr19.bam", motif_GCR, moi2, genome)
```
